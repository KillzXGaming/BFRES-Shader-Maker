...
// Wii U can only do 16 slots total, so do not bind specific slots and let the compiler handle it
#if (IS_WII_U == 1)
...
#define fViewPosZ fTexCoords0.z
...
float MultiplyAdd(float val, float mul, float add)
{
    return val * mul + add;
}

vec3 MultiplyAdd(vec3 val, vec3 mul, vec3 add)
{
    return val * mul + add;
}
...
void Indirect(inout vec2 t_TexCoord, bool is_indirect, int select)
{
    ...
    vec2 t_IndOffset = texture(indirect_texture, SelectTexCoord(texcoord_select_indirectA)).rg;
...
vec4 SampleMultiTextureA() {
    vec2 t_TexCoord  = SelectTexCoord(texcoord_select_multiA);
...
vec4 SampleMultiTextureB() {
    vec2 t_TexCoord  = SelectTexCoord(texcoord_select_multiB);
...
float GetGeoAlpha(sampler2D sampler)
{
    if (geo_multi_alpha_type == 1)  //texture with uv 0
       return texture(sampler, SelectTexCoord(0)).a;
    else if (geo_multi_alpha_type == 2) //texture with uv 2
       return texture(sampler, SelectTexCoord(2)).a;
    else if (geo_multi_alpha_type == 3) //texture with uv 3
       return texture(sampler, SelectTexCoord(3)).a;
...
{
    vec2 tex_coords = SelectTexCoord(texcoord_select_normal);
...
{
    vec2 tex_coords = SelectTexCoord(texcoord_select_normal2);
...
vec2 CalcSpecularFresnel(vec3 normal, vec3 view, float edge, float f_i, float f_s, float f_m, float roughness)
{
    float fresnel = saturate(4.0 * pow(edge, f_s * 6.0 + 2.0)) * f_i;
    float fresnel_m = fma(fresnel, -(1.0 - f_m), 1.0 - f_m) + f_m;

    float spec_intensity  = fma(fresnel_m * f_i, 0.5, fresnel_m);
    float spec_roughness = saturate(fma(fresnel, 0.4, roughness));

    float f_intensity      = f_i - fresnel;

    spec_intensity  = ( 0.5 * fresnel + 1.0 ) * mix( f_m, 1.0, ( 1.0 - f_intensity ) );
    spec_roughness  = saturate( fresnel * 0.4 + roughness );

    return vec2(spec_intensity, spec_roughness);
}

vec3 CalculateSpecular(vec3 t_Normal, vec3 t_ViewDirection, vec3 t_Lightprepass, 
            float shadow, vec3 indirect_bake_light)
{
    //roughness specular

    float t_Roughness = mat.specular_roughness;
    float spec_intensity = mat.specular_intensity;
    ...
    {
        vec2 spec_coords = SelectTexCoord(texcoord_select_specmask);
    ...
    if (enable_fresnel)
    {
        ...
        vec2 fresnel_output = CalcSpecularFresnel(t_Normal, t_ViewDirection.xyz, edge,
    ...
    if (enable_bake_l_cancel_s) //cancel spec by bake light and shadow
    {
    ...
    else if (enable_specular_shadow_link) //cancel spec by luminance spec and shadow
    {
...
vec3 CalculateLighting(vec3 N, float intensity)
{
    ...
    vec3 hemiLight = mix(environment.cHemiGroundColor.rgb * environment.cHemiGroundColor.w, environment.cHemiSkyColor.rgb * environment.cHemiGroundColor.w, hemiIntensity);
...
void DoAlphaTest(float t_Alpha)
{
    ...
    if (gsys_alpha_test_enable)
    {
        ...
        if (gsys_alpha_test_func == 0) //gequal
        {
            if (t_Alpha <= alpha_ref_value)
                discard;
        }
        else if (gsys_alpha_test_func == 1) //greater
        {
            if (t_Alpha > alpha_ref_value)
                discard;
        }
        else if (gsys_alpha_test_func == 2) //equal
        {
            if (t_Alpha == alpha_ref_value)
                discard;
        }
        ...
        else if (gsys_alpha_test_func == 6) //less
        {
            if (t_Alpha < alpha_ref_value)
                discard;
        }
...
float CalculateAlpha()
{
    ...
    {
        vec2 texCoords = SelectTexCoord(texcoord_select_albedo);
        ...
        albedo_sample.a = saturate(albedo_sample.a);

        return albedo_sample.a;
    }
    return 1.0;
}
...
void main()
{
    ...
    float NdotV = dot(normalize(fNormals.xyz), t_ViewDirection.xyz);
    ...
    if (enable_diffuse2 && enable_albedo || color_buffer_as_albedo)
    {
        ...
        t_Albedo.rgb = albedo_sample.rgb * scene_material.lighting.z * mat.albedo_tex_color.rgb;
    }

    if (ENABLE_TRANSPARENCY || enable_vtx_alpha_trans || gsys_alpha_test_enable)
        t_Alpha *= fVertexAlpha; //todo check the option this gets used in
    ...
    {
        vec2 emssion_coords = SelectTexCoord(texcoord_select_emission);
        Indirect(emssion_coords, indirect_effect_emission, select_indirect_mag_emission);

        vec4 emission_map = texture(emissive_map, SelectTexCoord(texcoord_select_emission)).rgba;
    ...
    if (enable_opa_trans)
    {
        ...
        //lighting but with negative normals
        vec3 t_Lightmap_DiffuseTrans = textureLod(gsys_lightmap_diffuse, -(t_FragNormal * normals_direction), t_LightIntensity * mat.transmit_shadow_intensity).rgb;

#if (BLENDER_RENDER == 1)
    t_Lightmap_DiffuseTrans = CalculateLighting(-t_FragNormal * normals_direction, t_LightIntensity * mat.transmit_shadow_intensity);
#endif
    ...
    if (enable_depth_buffer)
    {
        ...
        float z_dist = (context.cNearFar.y - context.cNearFar.y);
    ...
    //OUTPUT
    vec3 t_OutputColor = vec3(0);

    t_OutputColor.rgb += t_Albedo;

    if (enable_diffuse)
        t_OutputColor.rgb *= t_IncomingLightDiffuse;

    // If not albedo ao, multiply ao to the output color
    if (!enable_albedo_ao)
    ...
    if (enable_game_effect) //Game color effect (star power up, ink effect)
    ...
    if (enable_decal_ao)
    {
        ...
        if (decal_trail_type == 1) //Light
        {
            t_TrailValue = saturate(max(t_TrailValue * 2.0, 0.1));
            t_OutputColor.rgb = mix(vec3(0.0), t_OutputColor.rgb, t_TrailValue);
        }
        else if (decal_trail_type == 2) //Ice
        {
           // t_TrailValue = saturate(t_TrailValue * -2.0 + 1.9);
           // t_OutputColor.rgb = mix(t_OutputColor.rgb, vec3(0.0), t_TrailValue);
        }
        else if (decal_trail_type == 3) 
        {
          //  t_TrailValue = max(min(t_TrailValue * 2.0, 1.0), 0.1);
            //t_OutputColor.rgb = mix(mat.decal_trail_color.rgb, t_OutputColor.rgb, t_TrailValue);
        }
        else if (decal_trail_type == 4) //Used for snow
        {
            float t_TrailValue =  texture(gsys_static_depth_shadow, screenCoords).w;
        ...
        else if (decal_trail_type == 5) //Emission trail for anti g
        {
    ...
    {
        out_attr0 = vec4(0.0); //nothing but discard alpha
    }
    else
    {
        ...
        //t_OutputColor.rgb = textureLod(gsys_lightmap_diffuse, t_FragNormal * vec3(1, 1, -1), t_LightIntensity).rgb;
        vec4 albedo_sample = texture(normal_map, fTexCoords0.xy);
    ...
    {           
        ...
        else if (type == 8) // Normal Map 1
        {
            out_attr0.rgb = SampleNormalMap0().rgb;
        }     
        else if (type == 9) // Tangents
        {
            out_attr0.rgb = fTangents.rgb * 0.5 + 0.5;
        }    
        else if (type == 10) // Alpha
        {
        ...
        else if (type == 12) // Indirect RGB
        {
            out_attr0.rgb = t_BakeResult.IndirectLight.rgb;
        }   
        else if (type == 12) // Indirect A
        {
#if (BLENDER_RENDER == 1)
  ...
  //  vec3 t_Reflect = reflect(t_ViewDirection.xyz, normalize(t_FragNormal.rgb)); // reflection
	//vec4 specular_cubemap = textureLod(gsys_cube_map, vec4(t_Reflect.rgb , 0.0), 0.0);	
    //out_attr0.rgb = convert_cubemap_hdr(specular_cubemap).rgb;
    return;
}
