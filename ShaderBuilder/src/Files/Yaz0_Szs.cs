...
{
#if WIN32
    public const string DllName = "libszs_win32.dll";
#else
    public const string DllName = "libszs";
#endif
...
public static class szs
{
    ...
    {
        if (File.Exists("libszs.dll") && Environment.Is64BitProcess && RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            return true;
        if (File.Exists("libszs_32.dll") && RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            return true;
        if (File.Exists("libszs.so") && RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
            return true;
        return false;
    }
    ...
    public static byte[] Encode(byte[] data, CompressionAlgorithm algorithm = CompressionAlgorithm.MK8, uint alignment = 0)
    {
        ...
        try
        {
            ...
            //Set alignment
            encodedData[8]  = (byte)((alignment >> 24) & 0xFF);
            encodedData[9]  = (byte)((alignment >> 16) & 0xFF);
            encodedData[10] = (byte)((alignment >> 8)  & 0xFF);
            encodedData[11] = (byte)((alignment >> 0)  & 0xFF);
