...
namespace CafeLibrary
{
    ...
    public static class SARC_Parser
    {
        ...
        {
            string folder = Path.Combine("lib", "Hashes");
        ...
        public static bool IsSarc(Stream stream)
        {
        ...
        public static bool GuessAlignmentExt(string name, out uint align)
        {
            align = 0;
            if (name.EndsWith(".bfttf")) align = 0x2000;
            if (name.EndsWith(".bfotf")) align = 0x2000;
            return align != 0;
        }
        ...
        public static uint GuessAlignment(Dictionary<string, byte[]> files)
        {
            ...
            {
                uint fileRes = GuessFileAlignment(f.Value);
                if (GuessAlignmentExt(f.Key, out uint align))
                    fileRes = align;
        ...
        public static uint GuessFileAlignment(byte[] f)
        {
            ...
            else if (f.Matches("MsgStdBn") || f.Matches("MsgPrjBn")) return 256;
        ...
        public static Tuple<int, byte[]> PackN(SarcData data, int _align = -1)
        {
            ...
            foreach (string k in Keys)
            {
                if (GuessAlignmentExt(k, out uint a))
                    bw.Align((int)a);
                else
        ...
        public static SarcData UnpackRamN(Stream src, bool leaveOpen = false)
        {
            ...
            if (Encoding.ASCII.GetString(br.ReadBytes(4)) != "SARC")
